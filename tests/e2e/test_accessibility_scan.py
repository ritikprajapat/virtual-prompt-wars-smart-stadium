"""Runs axe-core against the fan assistant and staff dashboard pages, asserting zero violations."""
import pytest
...
def test_page_has_no_axe_violations(page, live_server, path):
    ...
    details = "\n".join(f"{v['id']}: {v['description']} ({len(v['nodes'])} nodes)" for v in violations)
...
def test_tab_order_reaches_wayfinding_form_before_other_panels(page, live_server):
    ...
    for _ in range(6):
        page.keyboard.press("Tab")
        focused_ids.append(page.evaluate("document.activeElement.id || document.activeElement.tagName"))
